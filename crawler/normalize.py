...
def recall_score(article: dict[str, Any], row: dict[str, Any]) -> int:
    ...
    Location facets (`Srbija`, `Beograd`) are true of almost every Serbian domestic
    story, so they rank a candidate but cannot shortlist it when the keyword also
    has a topic facet. Otherwise a "Serbian PM" subscription sends every flood and
    sports story to the model. A keyword that *is* just a place (the user subscribed
    to Serbia) still shortlists on the place.

    The same rule applies to loose `search_terms`: a bare generic like `Srbija`
    cannot carry the shortlist when more specific terms exist.

    Ranking: topic hits are worth ten location hits, so a limited scoring budget is
    spent on the strongest candidates first. This never decides relevance — only
    whether the article is worth an LLM call.
    ...
    strict rule path / AI prompt. False-friend token guards still apply.
    """
